...
export function runBotPlacement(state, botKey, difficulty, rng) {
  ...
  // 1. Place cards from hand into empty slots.
  while (p.hand.length > 0 && emptySlots(p.board).length > 0) {
    const slot = level >= 2 ? bestEmptySlot(p.board) : rng.pick(emptySlots(p.board));
    G.placeCard(state, botKey, 0, slot);
  }
...
function tryStrategicMerge(state, botKey, rng, level) {
  ...
  // Prefer merging two lowest-tier cards if it forms something stronger and
  // board has few empty slots left (Expert also merges to dodge death).
  ...
  if (shouldMerge && sorted.length >= 2) {
    G.mergeCards(state, botKey, sorted[0], sorted[1]);
  }
}
...
function maybeUseSpellsAndChips(state, botKey, enemyKey, rng, level) {
  ...
  for (const spell of p.spells.slice()) {
    if (spell.dmg) {
      ...
      if (killTarget !== undefined && (level >= 2 || rng.next() < 0.5)) {
        G.castSpell(state, botKey, spell.id, enemyKey, killTarget);
  ...
  if (level === 3) {
    ...
    if (hurt !== undefined) {
      ...
      if (healSpell && p.board[hurt].hp <= p.board[hurt].maxHp * 0.4) {
        G.castSpell(state, botKey, healSpell.id, botKey, hurt);
        return;
      }
    }
  }
  ...
  if (p.chips.length > 0) {
    ...
    if (myFilled.length > 0) {
      ...
      G.attachChip(state, botKey, p.chips[0].id, botKey, target);
    }
  }
}
...
function assignSmartDefense(state, botKey, rng) {
  ...
  fragile.forEach(slot => G.setDefend(state, botKey, slot));
}

export function runBotAttack(state, botKey, difficulty, rng) {
  const p = state.players[botKey];
  const enemyKey = state.order.find(k => k !== botKey);
  const enemy = state.players[enemyKey];
  const level = DIFFICULTIES.indexOf(difficulty);
  ...
  attackers.forEach(slot => {
    ...
    G.setAttack(state, botKey, slot, enemyKey, targetSlot);
  });
  G.readyAttack(state, botKey);
}
