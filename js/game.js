...
// v2.2: HAND_SIZE caps how many *Blue* (placeable) cards a hand can hold,
// not the hand's raw length. Non-Blue "blueprint" cards can never be
// placed, so if they counted against the same cap, a run of bad luck (a
// few non-Blue draws in a row) could permanently fill every hand slot with
// cards that can never be placed - with nothing on the board left to merge
// them into either, that's an unrecoverable softlock, not just a slow
// game. Blueprints still occupy a hand slot once drawn (they're visible
// and can't be discarded), they just don't stop fresh Blues from coming.
function blueCountInHand(playerState) {
  return playerState.hand.filter(c => c.tier === 1).length;
}

function drawToHand(state, playerKey, count) {
  const p = state.players[playerKey];
  for (let i = 0; i < count; i++) {
    if (blueCountInHand(p) >= HAND_SIZE) break;
...
function abilityTrigger(state, playerKey, card, trigger, slot, deathSource) {
  ...
  if (trigger === 'onplay') {
    if (a === 'onplay_dmg2' || a === 'onplay_dmg1') {
      ...
      discardRandomFromHand(state, enemyKey, source);
...
function drawOneCard(state, playerKey) {
  const p = state.players[playerKey];
  if (blueCountInHand(p) < HAND_SIZE && p.deck.length > 0) {
    p.hand.push(p.deck.shift());
    pushFx(state, { type: 'draw', owner: playerKey });
  }
}
function discardRandomFromHand(state, playerKey, source) {
  const p = state.players[playerKey];
  if (p.hand.length === 0) return;
  const idx = Math.floor(state._rng.next() * p.hand.length);
  const [card] = p.hand.splice(idx, 1);
  pushFx(state, { type: 'discard', owner: playerKey, cardName: card.name, source });
  pushLog(state, `${card.name} discarded from ${playerKey}'s hand`);
}
...
function killCard(state, ownerKey, slot, deathSource) {
  ...
  const card = p.board[slot];
  if (!card) return;
  const canReplenish = card.tier === 1 && p.everMergedUp;
...
function mergeCards(state, playerKey, slotA, slotB) {
  ...
  p.everMergedUp = true;
  const canReplenish = bluesConsumed > 0; // p.everMergedUp is now true above, unconditionally true here
  ...
  // v2.2: if the player deck-built a matching-tier "blueprint" card and it's
  // sitting in their hand (it can never be placed directly, only merged
  // into), consume it and use its name/ability for the merge result instead
  // of a generic fusion - this is what makes picking higher-tier archetypes
  // in the deck builder meaningful.
  const blueprintIdx = p.hand.findIndex(c => c.tier === newTier);
  let mergedName, mergedAbility;
  if (blueprintIdx !== -1) {
    const blueprint = p.hand.splice(blueprintIdx, 1)[0];
    mergedName = blueprint.name;
    mergedAbility = blueprint.ability;
  } else {
    mergedName = `${TIERS[newTier].name} Fusion`;
    mergedAbility = a.tier >= b.tier ? a.ability : b.ability;
  }
  ...
  pushFx(state, { type: 'merge', owner: playerKey, fromSlotA: slotA, fromSlotB: slotB, toSlot: slotA, resultTier: newTier, usedBlueprint: blueprintIdx !== -1 });
  abilityTrigger(state, playerKey, merged, 'onplay', slotA);
  pushLog(state, blueprintIdx !== -1
    ? `${playerKey} merges into ${merged.name} (tier ${newTier}, using a blueprint from hand)`
    : `${playerKey} merges into ${merged.name} (tier ${newTier})`);
...
function setDefend(state, playerKey, slot) {
  ...
  // v2.0: since every deck starts as 12 Blues, a player's Blues can't defend
  // until they've merged at least once - once that's happened, it's
...
// A hard backstop so no match can run forever. Blue replenishment is
// permanently unlocked the instant a player first merges (see
// `everMergedUp`), which - on rare seeds/matchups, most visible in bot-vs-
// bot play - can sustain a genuine mutual stalemate where neither side's
// total card count ever reaches zero. Rather than an open-ended AI
// rebalance, MAX_ROUNDS guarantees a bounded match length: past it, the
// player with the stronger overall position (board HP, cards left, unused
// spells/chips) wins outright, with a further tie counted as a draw.
const MAX_ROUNDS = 300;
function tiebreakScore(playerState) {
  const boardHp = playerState.board.reduce((sum, c) => sum + (c ? c.hp : 0), 0);
  const cardsLeft = boardCount(playerState) + playerState.hand.length + playerState.deck.length;
