...
function mergeCards(state, playerKey, slotA, slotB) {
  ...
  const blueprintIdx = p.deck.findIndex(c => c.tier === newTier);
  if (blueprintIdx === -1) {
  ...
  // BUGFIX: replenishment used to hand back one new Blue for EVERY Blue
  // consumed by the merge. For a Blue+Blue merge that meant 2 cards went
  // in, 1 merged card came out, AND 2 fresh Blues got added to the deck -
  // a net GAIN of one card out of thin air on every single fusion, which
  // (especially against merge-happy Hard/Expert/Master bots) grew both
  // players' total card counts faster than combat could ever shrink them,
  // so matches never actually ended. A merge should be net-neutral on
  // total card count (2 cards in, 1 merged card + at most 1 replenished
  // Blue out) - so replenish exactly 1 Blue whenever at least one Blue was
  // consumed, never bluesConsumed. Also gated on hasRemainingBlueprints
  // now (see replenishBlue's doc comment) - this merge just consumed the
  // blueprint above, so if that was the player's last one, no replenish.
  const canReplenish = bluesConsumed > 0 && hasRemainingBlueprints(p);
  if (canReplenish) replenishBlue(state, playerKey, 1);
