...
const TUTORIAL_STEPS = [
  ...
  {
    text: 'New round! Instead of attacking, try tapping one of your cards, then hitting Defend to block incoming damage.',
...
function loadBeatenDifficulties() {
  try {
    ...
    // One-time migration from the earlier "beat Master unlocks Pink" scheme,
    // so anyone who already earned it under the old rules doesn't lose it.
...
function recordDifficultyBeaten(diff) {
  ...
  const wasPinkUnlocked = isPinkUnlocked(set.filter(d => d !== diff));
  const wasFlameUnlocked = isFlameUnlocked(set.filter(d => d !== diff));
  const wasSovereignUnlocked = isSovereignUnlocked(set.filter(d => d !== diff));
  updateThemeButtons();
  if (!wasPinkUnlocked && isPinkUnlocked(set)) { showToast('💗 Pink Mode unlocked! Check Options to try it.', 2600); Sound.sparkle(); }
  if (!wasSovereignUnlocked && isSovereignUnlocked(set)) { showToast('👑 Sovereign theme unlocked! Check Options to try it.', 2600); Sound.sparkle(); }
  if (!wasFlameUnlocked && isFlameUnlocked(set)) { showToast('🔥 Flame theme unlocked! You beat every difficulty!', 3000); Sound.sparkle(); }
}
function isPinkUnlocked(set) { return (set || loadBeatenDifficulties()).includes('Medium'); }
function isSovereignUnlocked(set) { return (set || loadBeatenDifficulties()).includes('Master'); }
function isFlameUnlocked(set) {
  const beaten = set || loadBeatenDifficulties();
  return ALL_DIFFICULTIES.every(d => beaten.includes(d));
}
const THEME_UNLOCK_CHECK = { dark: () => true, light: () => true, pink: () => isPinkUnlocked(), flame: () => isFlameUnlocked(), sovereign: () => isSovereignUnlocked() };
...
function applyTheme(theme) {
  ...
  currentTheme = theme;
  document.documentElement.classList.remove('theme-light', 'theme-pink', 'theme-flame', 'theme-sovereign');
...
function updateThemeButtons() {
  ...
  ['dark', 'light', 'pink', 'flame', 'sovereign'].forEach(t => {
  ...
  const pinkBadge = document.getElementById('medium-pink-badge');
  if (pinkBadge) {
    const unlocked = isPinkUnlocked(beaten);
    pinkBadge.classList.toggle('unlocked', unlocked);
    pinkBadge.title = unlocked ? 'Pink Mode unlocked! Try it in Options.' : 'Beat Medium to unlock Pink Mode!';
  }
  const sovereignBadge = document.getElementById('master-sovereign-badge');
  if (sovereignBadge) {
    const unlocked = isSovereignUnlocked(beaten);
    sovereignBadge.classList.toggle('unlocked', unlocked);
    sovereignBadge.title = unlocked ? 'Sovereign theme unlocked! Try it in Options.' : 'Beat Master to unlock the Sovereign theme!';
  }
  ...
  if (flameHint) {
    flameHint.textContent = isFlameUnlocked(beaten)
      ...
      : `🔥 Beat every difficulty at least once to unlock a secret 4th theme. (${beaten.length}/5)`;
  }
}
