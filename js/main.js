...
function showScreen(id) {
  ...
  if (id === 'screen-menu') {
    updateDailyChallengeStripSub();
    updateAchievementsStripSub();
  }
}
...
/* ============================================================
   ...
   independent of it. Tutorial actions never count.
   ============================================================ */
...
function progressDailyChallenge(type, amount = 1) {
  ...
  else updateDailyChallengeStripSub();
}
...
function updateDailyChallengeStripSub() {
  const el = document.getElementById('daily-challenge-strip-sub');
  if (!el) return;
  const s = loadDailyChallengeState();
  if (s.claimed) { el.textContent = 'Completed today ✓'; el.classList.add('ready'); }
  else { const def = getDailyChallengeDef(); el.textContent = `${s.progress}/${def.target} today`; el.classList.remove('ready'); }
}
function openDailyChallenge() {
  const s = loadDailyChallengeState();
  const def = getDailyChallengeDef();
  const pct = Math.round(Math.min(100, (s.progress / def.target) * 100));
  const old = document.getElementById('daily-challenge-overlay');
  if (old) old.remove();
  const overlay = document.createElement('div');
  ...
  overlay.innerHTML = `
    <div class="feature-panel">
      <button class="feature-close">✕</button>
      <span class="feature-kicker">TODAY'S FORGE WORK</span>
      <h2>DAILY CHALLENGE</h2>
      <p>${def.desc}</p>
      <div class="challenge-progress-track"><div class="challenge-progress-fill" style="width:${pct}%"></div></div>
      <div class="challenge-progress-label"><span>${s.progress}/${def.target}</span><span>${s.claimed ? 'CLAIMED ✓' : pct + '%'}</span></div>
      <div class="challenge-streak-pill">🔥 ${s.streak || 0}-day streak</div>
      ${s.claimed
        ? '<p class="sub small" style="margin-top:14px;">Come back after midnight for a fresh challenge.</p>'
        : `<p class="sub small" style="margin-top:14px;">Reward on completion: ${dailyChallengeReward((s.streak || 0) + 1)} Bux. Progress tracks automatically as you play — no need to start anything.</p>`}
    </div>`;
  document.body.appendChild(overlay);
  overlay.querySelector('.feature-close').onclick = () => overlay.remove();
  overlay.onclick = e => { if (e.target === overlay) overlay.remove(); };
}
document.getElementById('btn-daily-challenge')?.addEventListener('click', openDailyChallenge);

/* ============================================================
   NEW FEATURE 2: FORGE MILESTONES (achievements)
   A fixed set of one-time milestones layered on top of the
   existing Quests (which only ever track "beat this difficulty").
   These cover a wider range of play - first actions, totals,
   wagers, collection completion, and the new Daily Challenge -
   each with its own one-time Bux reward. checkAchievements() is
   deliberately idempotent (safe to call repeatedly from several
   places) and never calls anything that could loop back into it.
   ============================================================ */
const ACHIEVEMENTS = [
...
function updateAchievementsStripSub() {
  const el = document.getElementById('achievements-strip-sub');
  if (!el) return;
  el.textContent = `${loadUnlockedAchievements().length}/${ACHIEVEMENTS.length} unlocked`;
}
function openAchievements() {
  const unlocked = loadUnlockedAchievements();
  const old = document.getElementById('achievements-overlay');
  if (old) old.remove();
  const overlay = document.createElement('div');
  ...
  overlay.innerHTML = `
    <div class="feature-panel" style="max-height:82vh; overflow-y:auto;">
      <button class="feature-close">✕</button>
      <span class="feature-kicker">${unlocked.length}/${ACHIEVEMENTS.length} UNLOCKED</span>
      <h2>FORGE MILESTONES</h2>
      <div class="ledger-list" style="margin-top:10px;">
        ${ACHIEVEMENTS.map(a => {
