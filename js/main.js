...
const WAGER_CAPS = Object.freeze({ Easy: 10, Medium: 20, Hard: 40, Expert: 75, Master: 100 });
...
document.querySelectorAll('#shop-diff-list .diff-card').forEach(btn => {
  ...
  btn.addEventListener('click', () => {
    ...
    const cap = WAGER_CAPS[diff] || MAX_MULTIPLAYER_WAGER;
    ...
    if (amt > cap) {
      ...
      document.querySelectorAll('#wager-presets .wager-btn').forEach(b => b.classList.toggle('active', Number(b.dataset.amt) === cap));
      return;
    }
    if (amt > loadBux()) {
