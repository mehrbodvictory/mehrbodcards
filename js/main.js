...
const MIN_BUX_FLOOR = 10; // spending (packs, cosmetics, or a wager) can never drop a player below this
function loadBux() {
  try {
...
function spendBux(amount) {
  const bal = loadBux();
  if (!(amount > 0) || bal - amount < MIN_BUX_FLOOR) return false;
  saveBux(bal - amount);
  updateBuxDisplay();
  return true;
}
...
function saveOwnedCosmetics(list) {
  try { localStorage.setItem('mehrbod-cards-owned-cosmetics', JSON.stringify(list)); } catch (e) {}
}
...
function equipSleeve(id) {
  try { localStorage.setItem('mehrbod-cards-equipped-sleeve', id); } catch (e) {}
  document.documentElement.classList.remove('sleeve-holo', 'sleeve-gold');
...
function renderCosmeticsShop() {
  ...
  // A free, always-available "no sleeve" option alongside the purchasable ones.
  const classicRow = document.createElement('button');
  classicRow.className = 'menu-card cosmetic-card owned';
  classicRow.innerHTML = `
    <span class="menu-card-title">🃏 Classic Sleeves</span>
    <span class="menu-card-sub">The default look — free.</span>
    <span class="cosmetic-status">${equippedSleeve === 'none' ? 'Equipped ✓' : 'Tap to equip'}</span>`;
  classicRow.addEventListener('click', () => equipSleeve('none'));
  list.appendChild(classicRow);

  COSMETIC_ITEMS.forEach(item => {
    const owned = ownsCosmetic(item.id);
    const equipped = item.kind === 'sleeve' && equippedSleeve === item.id;
    const row = document.createElement('button');
    row.className = 'menu-card cosmetic-card' + (owned ? ' owned' : '');
    let status;
    if (!owned) status = `${item.cost.toLocaleString()} Bux`;
    else if (item.kind === 'sleeve') status = equipped ? 'Equipped ✓' : 'Owned — tap to equip';
    else status = 'Owned ✓';
    row.innerHTML = `
      <span class="menu-card-title">${item.name}</span>
      <span class="menu-card-sub">${item.desc}</span>
      <span class="cosmetic-status">${status}</span>`;
    row.addEventListener('click', () => buyOrEquipCosmetic(item));
...
// supplies onConfirm(config), invoked with { unitIds, spellIds, chipIds }
// once the player has picked exactly 12 owned units (any tier), 4 owned
// spells, and 2 owned chips. `unitIds` may repeat the same archetype id
// multiple times - the deck builder's unit picker is a quantity stepper,
// not a single-pick list, since a new player only owns the 5 free Blues
// and needs to fill all 12 slots from just those until they buy Card Packs.
let deckBuilderOnConfirm = null;
let deckBuilderSelSpells = new Set();
let deckBuilderSelChips = new Set();
let deckBuilderUnitCounts = {}; // archetypeId -> count

function totalUnitsSelected() {
  return Object.values(deckBuilderUnitCounts).reduce((sum, n) => sum + n, 0);
}

function loadLastDeckConfig() {
  try {
    const raw = JSON.parse(localStorage.getItem('mehrbod-cards-last-deck') || 'null');
    if (raw && Array.isArray(raw.unitIds) && Array.isArray(raw.spellIds) && Array.isArray(raw.chipIds)) return raw;
  } catch (e) {}
  return null;
}
function saveLastDeckConfig(config) {
  try { localStorage.setItem('mehrbod-cards-last-deck', JSON.stringify(config)); } catch (e) {}
}

// Fills in any still-empty slots with sensible defaults, so a first-time
// (or otherwise incomplete) deck is always immediately confirmable without
// forcing the player to manually build one from nothing every match.
function autoFillDeckDefaults() {
  const owned = loadCollection();
  if (deckBuilderSelSpells.size < 4) {
    for (const id of owned.spells) {
      if (deckBuilderSelSpells.size >= 4) break;
      deckBuilderSelSpells.add(id);
    }
  }
  if (deckBuilderSelChips.size < 2) {
    for (const id of owned.chips) {
      if (deckBuilderSelChips.size >= 2) break;
      deckBuilderSelChips.add(id);
    }
  }
  const remaining = 12 - totalUnitsSelected();
  if (remaining > 0) {
    const firstBlue = UNIT_ARCHETYPES[1][0].id;
    deckBuilderUnitCounts[firstBlue] = (deckBuilderUnitCounts[firstBlue] || 0) + remaining;
  }
}
...
function openDeckBuilder(onConfirm) {
  ...
  const owned = loadCollection();
  const last = loadLastDeckConfig();
  deckBuilderUnitCounts = {};
  if (last) {
    deckBuilderSelSpells = new Set(last.spellIds.filter(id => owned.spells.includes(id)));
    deckBuilderSelChips = new Set(last.chipIds.filter(id => owned.chips.includes(id)));
    last.unitIds.forEach(id => {
      if (isUnitArchetypeOwned(id)) deckBuilderUnitCounts[id] = (deckBuilderUnitCounts[id] || 0) + 1;
    });
  } else {
    deckBuilderSelSpells = new Set();
    deckBuilderSelChips = new Set();
  }
  autoFillDeckDefaults();
  renderDeckBuilder();
  showScreen('screen-deck-builder');
}

// A short one-line description for a unit archetype's ability, for display
// in the deck builder / collection book - the "classic" archetypes (Warden/
// Wraith/Colossus) roll a random ability from a pool each time they're
// drawn, so they get a generic note instead of a single fixed description.
function archetypeAbilityText(archetype) {
  if (archetype.pool.length === 1) {
    const id = archetype.pool[0];
    return id === 'none' ? 'No special ability' : (ABILITY_SHORT[id] || '');
  }
  return 'Random ability each time it\u2019s drawn';
}

function renderDeckBuilderUnits() {
  const el = document.getElementById('deck-builder-units');
  if (!el) return;
  el.innerHTML = '';
  [1, 2, 3, 4].forEach(tier => {
    UNIT_ARCHETYPES[tier].forEach(a => {
      if (!isUnitArchetypeOwned(a.id)) return; // unowned cards don't clutter the deck builder - see the Collection book instead
      const count = deckBuilderUnitCounts[a.id] || 0;
      const div = document.createElement('div');
      div.className = 'dcard unit-dcard tier' + tier + (count > 0 ? ' selected' : '');
      div.innerHTML = `
        <div class="dcard-name">${a.name}</div>
        <div class="dcard-text">${archetypeAbilityText(a)}</div>
        <div class="dcard-stepper">
          <button type="button" class="stepper-btn" data-act="minus">−</button>
          <span class="stepper-count">${count}</span>
          <button type="button" class="stepper-btn" data-act="plus">+</button>
        </div>`;
      div.querySelector('[data-act="minus"]').addEventListener('click', (e) => {
        e.stopPropagation();
        if ((deckBuilderUnitCounts[a.id] || 0) > 0) {
          deckBuilderUnitCounts[a.id]--;
          renderDeckBuilder();
        }
      });
      div.querySelector('[data-act="plus"]').addEventListener('click', (e) => {
        e.stopPropagation();
        if (totalUnitsSelected() >= 12) { showToast('Your deck already has all 12 unit slots filled.'); return; }
        deckBuilderUnitCounts[a.id] = (deckBuilderUnitCounts[a.id] || 0) + 1;
        renderDeckBuilder();
      });
      el.appendChild(div);
    });
  });
  document.getElementById('deck-builder-unit-count').textContent = `${totalUnitsSelected()}/12`;
}
...
function renderDeckBuilder() {
  ...
  renderDeckBuilderUnits();

  SPELL_DEFS.forEach(s => {
    const isOwned = owned.spells.includes(s.id);
    if (!isOwned) return; // unowned cards don't clutter the deck builder - see the Collection book instead
    const isSel = deckBuilderSelSpells.has(s.id);
    const el = document.createElement('button');
    el.className = 'dcard' + (isSel ? ' selected' : '');
  ...
  document.getElementById('btn-deck-builder-confirm').disabled =
    !(deckBuilderSelSpells.size === 4 && deckBuilderSelChips.size === 2 && totalUnitsSelected() === 12);
}

document.getElementById('btn-deck-builder-confirm').addEventListener('click', () => {
  if (deckBuilderSelSpells.size !== 4 || deckBuilderSelChips.size !== 2 || totalUnitsSelected() !== 12) return;
  const unitIds = Object.entries(deckBuilderUnitCounts).flatMap(([id, count]) => Array(count).fill(id));
  const config = { unitIds, spellIds: Array.from(deckBuilderSelSpells), chipIds: Array.from(deckBuilderSelChips) };
  saveLastDeckConfig(config);
  const fn = deckBuilderOnConfirm;
  deckBuilderOnConfirm = null;
  if (fn) fn(config);
});

// ---- Collection book (v2.2) ------------------------------------------------
// A read-only reference showing every card in the game - owned ones look
// normal, uncollected ones are greyed out with a lock, same visual language
// as the deck builder's locked state used to have before unowned cards were
// pulled out of that screen entirely.
...
document.getElementById('btn-rules').addEventListener('click', () => startFullTutorial());
...
document.getElementById('bux-counter').addEventListener('click', () => { updateBuxDisplay(); showScreen('screen-shop'); });
...
document.getElementById('btn-shop-cosmetics').addEventListener('click', () => {
  updateBuxDisplay();
  renderCosmeticsShop();
...
document.querySelectorAll('#shop-diff-list .diff-card').forEach(btn => {
  ...
  btn.addEventListener('click', () => {
    ...
    openDeckBuilder((config) => {
      if (!spendBux(amt)) { showToast("You don't have enough Mehrbod Bux for that wager (you always keep at least 10)."); return; }
      botDifficulty = diff;
...
function themeDisplayName(t) {
  return { dark: 'Dark', light: 'Light', verdant: 'Verdant', pink: 'Pink', storm: 'Storm', aurora: 'Aurora', sovereign: 'Sovereign', flame: 'Flame', mrmoney: 'Mr Money' }[t] || t;
}
...
const ALL_THEME_NAMES = ['dark', 'light', 'verdant', 'pink', 'storm', 'aurora', 'sovereign', 'flame', 'mrmoney'];
function loadTheme() {
  try {
...
// ---- First-run tutorial ------------------------------------------------
// Fires exactly once, ever, per browser - first impressions matter, and a
// brand-new player landing on the menu with no idea what any of it does is
// the single easiest way to lose them before their first match. Runs after
// every other init call above so the menu/DOM/theme state is fully settled
// before the spotlight overlay starts measuring element positions.
