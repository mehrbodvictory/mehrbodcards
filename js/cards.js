...
// ---- Deck building ---------------------------------------------------------
// v2.0: unit decks are Blue-only. Green/Red/Orange can never be placed or
// drawn directly anymore - the only way to get one onto the board is by
// merging Blues (and their fusions) up through the tiers. `config`, if
// given, is a player-chosen deck: { spellIds: [...4 ids], chipIds: [...2
// ids] } picked from cards the player owns (see the deck builder in
// main.js). Without a config (e.g. the bot's deck), spells/chips are drawn
// randomly from the full pool, same as the original random-draft behavior.
function buildDeck(rng, config) {
  let units = [];
  for (let i = 0; i < 12; i++) units.push(makeUnitCard(1, rng));
  ...
  return {
    units,                                            // 12 Blue unit cards, unseen until drawn
