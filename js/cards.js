...
// Red no longer defends at all, so onplay_shield1 (which only matters for a
// card that can defend) has moved to the Green pool instead, where a +1
// charge is a real, meaningful bonus now that Green's base is just one.
const ABILITY_POOL_BY_TIER = {
  1: ['none', 'none', 'none', 'onplay_dmg1'],
  2: ['none', 'none', 'onplay_dmg1', 'onplay_heal2', 'ondeath_heal1', 'onplay_shield1'],
  3: ['none', 'onplay_dmg2', 'ondeath_dmg2', 'onattack_pierce'],
...
function makeUnitCard(tier, rng, abilityId) {
  const t = TIERS[tier];
