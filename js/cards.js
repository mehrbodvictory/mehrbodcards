...
const UNIT_ARCHETYPES = {
  ...
  2: [ // Green
    { id: 'green_warden',     name: 'Green Warden',     pool: ['none', 'none', 'onplay_dmg1', 'onplay_heal2', 'ondeath_heal1', 'onplay_shield1'] },
  ...
  3: [ // Red
    { id: 'red_wraith',      name: 'Red Wraith',      pool: ['none', 'onplay_dmg2', 'ondeath_dmg2', 'onattack_pierce'] },
  ...
  4: [ // Orange
    { id: 'orange_colossus',   name: 'Orange Colossus',   pool: ['onplay_dmg2', 'ondeath_dmg2', 'onattack_pierce'] },
...
// turn up in hand as a "blueprint": merging two Blues (or fusing further)
// consumes a matching-tier blueprint from hand, if one's there, and uses
// its name/ability for the result instead of a generic fusion (see
// `mergeCards` in game.js). Without a config (e.g. the bot's deck), a
// weighted random spread across all four tiers is used instead, so bot
// matches still show off named higher-tier archetypes.
const RANDOM_DECK_TIER_WEIGHTS = [1, 1, 1, 1, 1, 1, 1, 1, 1, 2, 2, 3]; // 9 blue, 2 green, 1 red - kept Blue-heavy so the bot's simple hand management doesn't get clogged with unplaceable blueprints
...
function buildDeck(rng, config) {
  ...
  return {
    units,                                            // 12 unit cards of the player's choosing, unseen until drawn
    spells: spellPool.map(makeSpellOrChip),            // exactly 4 spells, in hand from game start
    chips: chipPool.map(makeSpellOrChip),              // exactly 2 chips, in hand from game start
  };
}
