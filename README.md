...
- **v2.0: Blue-only placement.** Every deck is 12 Blue units now — Green,
  ...
  effect worth knowing: since `nonBlueUnitsRemaining` starts at zero for
  both players until at least one merge has happened, Blue cards can't
  defend (and won't replenish on death/merge) until something higher-tier
  exists somewhere in your deck/hand/board — this is intentional, not a bug,
  and it reinforces that merging is the whole game now, not just an option.
...
- **v2.0: Mehrbod Bux & the Shop.** A first-time player starts with 50
  Mehrbod Bux (persisted in `localStorage`, shown on the main menu). The
  starter pack, granted automatically on first launch, unlocks a random
  legal set (4 spells, 2 chips) so a new player can deck-build immediately.
  The Mehrbod Shop lets you spend Bux on a Card Pack (unlocks 2 more
  spells/chips toward completing your collection) or wager Bux in a match
  against the bot or a friend — win doubles your stake, lose and it's gone,
  a draw refunds it. A host's wager amount is sent to the guest as part of
  the same handshake as their deck config; the guest is charged the same
  amount automatically if they can afford it (if not, they still play, just
  without stakes on their side).
...
- **Themes**: Dark and Light are available from the start; Verdant, Pink,
  ...
  *above* the UI with `mix-blend-mode: screen` so they read as a glowing
  overlay instead of disappearing behind opaque panels.
