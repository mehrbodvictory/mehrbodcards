...
- **v2.0: Blue-only placement.** Every deck is 12 Blue units now — Green,
  Red, and Orange can never be placed from hand or drawn from the deck
  directly anymore. The only way to get one onto the board is by merging
  Blues (and their fusions) upward, enforced by the engine itself
  (`placeCard` in `js/game.js` rejects anything above tier 1). One knock-on
  effect worth knowing: Blue cards can't defend (and won't replenish on
  death/merge) until a player has merged at least once - but once they have,
  it's unlocked permanently for the rest of that match, even if the merged
  card later dies. It's a one-time flag (`everMergedUp`), not a live
  recheck of what's currently on the board - this is intentional, and it
  reinforces that merging is the whole game now, not just an option.
- **v2.0: Deck builder.** Before every match — Practice vs Bot, a wagered
  match, hosting, or joining — you pick exactly 4 spells and 2 chips from
  your owned collection (`js/main.js`'s `openDeckBuilder`/`renderDeckBuilder`).
  A starter pack, granted automatically on first launch, unlocks a random
  legal set (4 spells, 2 chips) so a new player can deck-build immediately;
  more variety comes from Card Packs bought with Mehrbod Bux in the Shop.
  Your 12 Blue units are always auto-included and never customized, since
  every Blue archetype is mechanically identical. In multiplayer, both
  sides' chosen configs are exchanged as part of the connection handshake
  before the match seed is sent (see `js/network.js`), so a custom deck
  works correctly for both host and guest without breaking the deterministic
  sync the whole match engine relies on.
- **v2.0/2.1: Mehrbod Bux, the Shop, and wager matches.** A first-time
  player starts with 50 Mehrbod Bux, shown in a fixed counter in the
  top-right corner (visible on every menu, hidden during an actual match)
  that persists in `localStorage`. Tapping it jumps straight to the Shop.
  Wager matches live under **Practice vs Bot** and **Host Match** now (each
  asks Normal vs Wager up front) rather than in the Shop itself — win
  doubles your stake, lose and it's gone, a draw refunds it. A host's wager
  amount is sent to the guest as part of the same handshake as their deck
  config; the guest is charged the same amount automatically if they can
  afford it (if not, they still play, just without stakes on their side).
  The **Mehrbod Shop** itself is just Card Packs (unlock 2 more spells/chips
  toward completing your collection) and **Cosmetics** — the purchasable
  Mr Money theme (1000 Bux, green palette, raining money, separate from the
  quest-earned themes), Holographic/Gold card sleeves (animated outline
  cosmetics, deliberately built on the CSS `outline` property rather than
  `border`/`box-shadow` so they can never clash with the more important
  per-card states like defending or low-hp, which already use those), and a
  Confetti+ effect for a bigger win celebration.
...
- **Board size**: 6 slots per player. **Hand size**: 3 unit cards drawn at a
  time from your 12-card deck, which is always 12 Blues (see v2.0 note
  above). Spells (4) and chips (2) are chosen in the deck builder from your
  owned collection, separate from the 12 Blues, and available in full from
...
- **Themes**: Dark and Light are available from the start; Verdant, Pink,
  Storm, Aurora, Sovereign, and Flame are unlocked by beating each bot
  difficulty at least once (see the Quests panel). Storm (the Hard reward)
  is rendered as rainclouds and lightning bolts, with thunder strikes
  confined to the cloud band and the bolts themselves rather than flashing
  the whole screen. Aurora (Expert) layers a slowly-rotating galaxy core,
  a small orbiting solar system, drifting nebula ribbons, and a dense
  twinkling starfield.
  Flame (beat every difficulty) and Sovereign (Master) intentionally sit
  *above* the UI with `mix-blend-mode: screen` so they read as a glowing
  overlay instead of disappearing behind opaque panels. Mr Money is the odd
  one out — it's bought in the Shop for 1000 Bux rather than earned through
  a quest, and rains money bills over a green palette using the same
  proven layering as Flame/Sovereign/Aurora.
