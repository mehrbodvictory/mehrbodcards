...
- **Themes**: Dark and Light are available from the start; Verdant, Pink,
  ...
  is rendered as rainclouds and lightning bolts sitting *behind* the game's
  cards and panels (negative z-index) rather than blended on top of them, so
  thunder strikes never wash out the UI — the flash is confined to the cloud
  band and the bolts themselves. Aurora (Expert) layers a slowly-rotating
  galaxy core under drifting nebula ribbons and a dense twinkling starfield.
